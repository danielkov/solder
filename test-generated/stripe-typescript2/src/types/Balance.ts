...
export const BalanceSchema: TypedSchema<Balance> = typed<Balance>(object({
  ...
  get connectReserved() {
    return optional(array(BalanceAmountSchema));
  },
  get instantAvailable() {
    return optional(array(BalanceAmountNetSchema));
  },
  get issuing() {
    return optional(BalanceDetailSchema);
  },
  livemode: boolean(),
  object: string(),
  pending: array(BalanceAmountSchema),
  get refundAndDisputePrefunding() {
    return optional(BalanceDetailUngatedSchema);
  },
}));
