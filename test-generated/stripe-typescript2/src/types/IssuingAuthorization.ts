...
export const IssuingAuthorizationSchema: TypedSchema<IssuingAuthorization> = typed<IssuingAuthorization>(object({
  amount: number(),
  get amountDetails() {
    return optional(IssuingAuthorizationAmountDetailsSchema);
  },
  approved: boolean(),
  authorizationMethod: string(),
  ...
  get cardholder() {
    return optional(CardholderOrRefSchema);
  },
  created: number(),
  currency: string(),
  get fleet() {
    return optional(IssuingAuthorizationFleetDataSchema);
  },
  get fraudChallenges() {
    return optional(array(IssuingAuthorizationFraudChallengeSchema));
  },
  get fuel() {
    return optional(IssuingAuthorizationFuelDataSchema);
  },
  id: string(),
  livemode: boolean(),
  merchantAmount: number(),
  ...
  get networkData() {
    return optional(IssuingAuthorizationNetworkDataSchema);
  },
  object: string(),
  get pendingRequest() {
    return optional(IssuingAuthorizationPendingRequestSchema);
  },
  requestHistory: array(IssuingAuthorizationRequestSchema),
  status: string(),
  get token() {
    return optional(TokenOrRefSchema);
  },
  transactions: array(IssuingTransactionSchema),
  get treasury() {
    return optional(IssuingAuthorizationTreasurySchema);
  },
