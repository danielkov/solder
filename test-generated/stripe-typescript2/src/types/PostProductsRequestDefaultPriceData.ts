...
export const PostProductsRequestDefaultPriceDataSchema: TypedSchema<PostProductsRequestDefaultPriceData> = typed<PostProductsRequestDefaultPriceData>(object({
  currency: string(),
  get currencyOptions() {
    return optional(PostPricesPriceRequestCurrencyOptionsObjectSchema);
  },
  get customUnitAmount() {
    return optional(PostProductsRequestCustomUnitAmountSchema);
  },
  get metadata() {
    return optional(PostAccountsRequestMetadataObjectSchema);
  },
  get recurring() {
    return optional(PostCheckoutSessionsRequestLineItemsItemRecurringSchema);
  },
