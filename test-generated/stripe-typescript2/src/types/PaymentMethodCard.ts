...
export const PaymentMethodCardSchema: TypedSchema<PaymentMethodCard> = typed<PaymentMethodCard>(object({
  brand: string(),
  get checks() {
    return optional(PaymentMethodCardChecksSchema);
  },
  ...
  funding: string(),
  get generatedFrom() {
    return optional(PaymentMethodCardGeneratedCardSchema);
  },
  last4: string(),
  get networks() {
    return optional(NetworksSchema);
  },
  regulatedStatus: optional(string()),
  get threeDSecureUsage() {
    return optional(ThreeDSecureUsageSchema);
  },
  get wallet() {
    return optional(PaymentMethodCardWalletSchema);
  },
}));
