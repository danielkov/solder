...
export const IssuingCardSchema: TypedSchema<IssuingCard> = typed<IssuingCard>(object({
  ...
  id: string(),
  last4: string(),
  get latestFraudWarning() {
    return optional(IssuingCardFraudWarningSchema);
  },
  livemode: boolean(),
  ...
  get personalizationDesign() {
    return optional(PersonalizationDesignOrRefSchema);
  },
  get replacedBy() {
    return optional(ReplacedByOrRefSchema);
  },
  get replacementFor() {
    return optional(ReplacementForOrRefSchema);
  },
  replacementReason: optional(string()),
  secondLine: optional(string()),
  get shipping() {
    return optional(IssuingCardShippingSchema);
  },
  spendingControls: IssuingCardAuthorizationControlsSchema,
  status: string(),
  type: string(),
  get wallets() {
    return optional(IssuingCardWalletsSchema);
  },
}));
