...
export const SourceTransactionSchema: TypedSchema<SourceTransaction> = typed<SourceTransaction>(object({
  get achCreditTransfer() {
    return optional(SourceTransactionAchCreditTransferDataSchema);
  },
  amount: number(),
  get chfCreditTransfer() {
    return optional(SourceTransactionChfCreditTransferDataSchema);
  },
  created: number(),
  currency: string(),
  get gbpCreditTransfer() {
    return optional(SourceTransactionGbpCreditTransferDataSchema);
  },
  id: string(),
  livemode: boolean(),
  object: string(),
  get paperCheck() {
    return optional(SourceTransactionPaperCheckDataSchema);
  },
  get sepaCreditTransfer() {
    return optional(SourceTransactionSepaCreditTransferDataSchema);
  },
  source: string(),
  status: string(),
  type: string(),
