...
export const TaxTransactionSchema: TypedSchema<TaxTransaction> = typed<TaxTransaction>(object({
  ...
  id: string(),
  get lineItems() {
    return optional(LineItemsProperty4Schema);
  },
  livemode: boolean(),
  get metadata() {
  ...
  reference: string(),
  get reversal() {
    return optional(TaxProductResourceTaxTransactionResourceReversalSchema);
  },
  get shipFromDetails() {
    return optional(TaxProductResourceShipFromDetailsSchema);
  },
  get shippingCost() {
    return optional(TaxProductResourceTaxTransactionShippingCostSchema);
  },
  taxDate: number(),
  type: string(),
}));
