    ...
    S: Clone + Send + Sync + 'static,
{
    ...
    fn router(self) -> Router<S> {
        let get_bookings_handler = |ctx: RequestContext<S>, auth: Auth, Extension(service): Extension<Self>, axum::extract::Query(query): axum::extract::Query<GetBookingsQuery>
        | async move {
        ...
        let create_booking_handler = |ctx: RequestContext<S>, auth: Auth, Extension(service): Extension<Self>, axum::Json(body): axum::Json<crate::types::Booking>
        | async move {
        ...
        let get_booking_handler = |ctx: RequestContext<S>, auth: Auth, Extension(service): Extension<Self>
        | async move {
        ...
        let delete_booking_handler = |ctx: RequestContext<S>, auth: Auth, Extension(service): Extension<Self>
        | async move {
