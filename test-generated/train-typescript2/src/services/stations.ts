...
export class StationsService {
   ...
   */
  async getStations(params: { page?: number; limit?: number; coordinates?: string; search?: string; country?: string }): Promise<GetStationsUnion> {
    const path = '/stations';
      ...
      switch (response.status) {
        case 400: {
          try {
            ...
            if (e instanceof GetStationsBadRequestError) {
              throw e;
            }
        ...
        case 401: {
          try {
            ...
            if (e instanceof GetStationsUnauthorizedError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
        ...
        case 403: {
          try {
            ...
            if (e instanceof GetStationsForbiddenError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
        ...
        case 429: {
          try {
            ...
            if (e instanceof GetStationsTooManyRequestsError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
        ...
        case 500: {
          try {
            ...
            if (e instanceof GetStationsInternalServerErrorError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
