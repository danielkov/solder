...
export class TripsService {
   ...
   */
  async getTrips(params: { page?: number; limit?: number; origin: string; destination: string; date: string; bicycles?: boolean; dogs?: boolean }): Promise<GetTripsUnion> {
    const path = '/trips';
      ...
      switch (response.status) {
        case 400: {
          try {
            ...
            if (e instanceof GetTripsBadRequestError) {
              throw e;
            }
        ...
        case 401: {
          try {
            ...
            if (e instanceof GetTripsUnauthorizedError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
        ...
        case 403: {
          try {
            ...
            if (e instanceof GetTripsForbiddenError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
        ...
        case 429: {
          try {
            ...
            if (e instanceof GetTripsTooManyRequestsError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
        ...
        case 500: {
          try {
            ...
            if (e instanceof GetTripsInternalServerErrorError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
