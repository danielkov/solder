...
export class TicketsService {
   ...
   */
  async buyMuseumTickets(params: { body: BuyMuseumTickets }): Promise<MuseumTicketsConfirmation> {
    const path = '/tickets';
      ...
      switch (response.status) {
        case 400: {
          try {
            ...
            if (e instanceof BuyMuseumTicketsBadRequestError) {
              throw e;
            }
        ...
        case 404: {
          try {
            ...
            if (e instanceof BuyMuseumTicketsNotFoundError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
   ...
   */
  async getTicketCode(params: { ticketId: string }): Promise<TicketCodeImage> {
      ...
      switch (response.status) {
        case 400: {
          try {
            ...
            if (e instanceof GetTicketCodeBadRequestError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
        ...
        case 404: {
          try {
            ...
            if (e instanceof GetTicketCodeNotFoundError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
