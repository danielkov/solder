    ...
    S: Clone + Send + Sync + 'static,
{
    ...
    fn router(self) -> Router<S> {
        let create_embeddings_handler = |ctx: RequestContext<S>, auth: ApiKey, Extension(service): Extension<Self>, axum::Json(body): axum::Json<crate::types::CreateEmbeddingsRequest>
        | async move {
        ...
        let list_embeddings_models_handler = |ctx: RequestContext<S>, auth: ApiKey, Extension(service): Extension<Self>
        | async move {
