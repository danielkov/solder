...
export class FaqService {
   ...
   */
  async listFaqItems(params: { category?: string }): Promise<FaqList> {
    const path = '/faq';
      ...
      switch (response.status) {
        case 500: {
          try {
            ...
            if (e instanceof ListFaqItemsInternalServerErrorError) {
              throw e;
            }
   ...
   */
  async getFaqItem(params: { id: string }): Promise<FaqItem> {
      ...
      switch (response.status) {
        case 404: {
          try {
            ...
            if (e instanceof GetFaqItemNotFoundError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
