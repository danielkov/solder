...
export class PetsService {
   ...
   */
  async listPets(params: { limit?: number; offset?: number; status?: string }): Promise<PetList> {
    const path = '/pets';
      ...
      switch (response.status) {
        case 400: {
          try {
            ...
            if (e instanceof ListPetsBadRequestError) {
              throw e;
            }
        ...
        case 500: {
          try {
            ...
            if (e instanceof ListPetsInternalServerErrorError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
   ...
   */
  async createPet(params: { body: NewPet }): Promise<Pet> {
    const path = '/pets';
      ...
      switch (response.status) {
        case 400: {
          try {
            ...
            if (e instanceof CreatePetBadRequestError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
   ...
   */
  async exportPets(): Promise<Uint8Array> {
   ...
   * Export all pets data as a binary archive file
   */
  async exportPetsCsv(): Promise<Uint8Array> {
   ...
   */
  async getPetById(params: { petId: string }): Promise<Pet> {
      ...
      switch (response.status) {
        case 404: {
          try {
            ...
            if (e instanceof GetPetByIdNotFoundError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
   ...
   */
  async updatePet(params: { petId: string; body: UpdatePet }): Promise<Pet> {
    const path = `/pets/{petId}`.replace('{petId}', String(params.petId));
    const url = `${this.baseUrl}${path}`;
      ...
      switch (response.status) {
        case 404: {
          try {
            ...
            if (e instanceof UpdatePetNotFoundError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
   ...
   */
  async deletePet(params: { petId: string }): Promise<void> {
    const path = `/pets/{petId}`.replace('{petId}', String(params.petId));
    const url = `${this.baseUrl}${path}`;
      ...
      switch (response.status) {
        case 404: {
          try {
            ...
            if (e instanceof DeletePetNotFoundError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
   ...
   */
  async getPetDocuments(params: { petId: string; format?: string }): Promise<Uint8Array> {
   ...
   */
  async getPetPhoto(params: { petId: string }): Promise<Uint8Array> {
