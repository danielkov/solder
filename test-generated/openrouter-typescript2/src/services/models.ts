...
export class ModelsService {
   ...
   */
  async getModels(params: { category?: string; supportedParameters?: string }): Promise<ModelsListResponse> {
    const path = '/models';
      ...
      switch (response.status) {
        case 400: {
          try {
            ...
            if (e instanceof GetModelsBadRequestError) {
              throw e;
            }
        ...
        case 500: {
          try {
            ...
            if (e instanceof GetModelsInternalServerErrorError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
      ...
      switch (response.status) {
        case 500: {
          try {
            ...
            if (e instanceof ListModelsCountInternalServerErrorError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
      ...
      switch (response.status) {
        case 401: {
          try {
            ...
            if (e instanceof ListModelsUserUnauthorizedError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
        ...
        case 500: {
          try {
            ...
            if (e instanceof ListModelsUserInternalServerErrorError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
