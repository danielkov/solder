...
export class ParametersService {
   ...
   */
  async getParameters(params: { author: string; slug: string; provider?: string }): Promise<GetParametersResponse> {
      ...
      switch (response.status) {
        case 401: {
          try {
            ...
            if (e instanceof GetParametersUnauthorizedError) {
              throw e;
            }
        ...
        case 404: {
          try {
            ...
            if (e instanceof GetParametersNotFoundError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
        ...
        case 500: {
          try {
            ...
            if (e instanceof GetParametersInternalServerErrorError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
