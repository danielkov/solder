...
export class EndpointsService {
      ...
      switch (response.status) {
        case 500: {
          try {
            ...
            if (e instanceof ListEndpointsZdrInternalServerErrorError) {
              throw e;
            }
   ...
   */
  async listEndpoints(params: { author: string; slug: string }): Promise<EndpointsListEndpointsResponse> {
      ...
      switch (response.status) {
        case 404: {
          try {
            ...
            if (e instanceof ListEndpointsNotFoundError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
        ...
        case 500: {
          try {
            ...
            if (e instanceof ListEndpointsInternalServerErrorError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
