...
export class AnalyticsService {
   ...
   */
  async getUserActivity(params: { date?: string }): Promise<GetUserActivityResponse> {
    const path = '/activity';
      ...
      switch (response.status) {
        case 400: {
          try {
            ...
            if (e instanceof GetUserActivityBadRequestError) {
              throw e;
            }
        ...
        case 401: {
          try {
            ...
            if (e instanceof GetUserActivityUnauthorizedError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
        ...
        case 403: {
          try {
            ...
            if (e instanceof GetUserActivityForbiddenError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
        ...
        case 500: {
          try {
            ...
            if (e instanceof GetUserActivityInternalServerErrorError) {
              throw e;
            }
            await this.raise(new UnexpectedError(response.status, await response.text()));
          }
