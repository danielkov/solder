import { array, literal, number, object, optional, string, typed } from '@speakeasy-api/tonic';
...
export const DataPropertySchema: TypedSchema<DataProperty> = typed<DataProperty>(object({
  ...
  created: number(),
  get error() {
    return optional(ErrorProperty2Schema);
  },
  id: string(),
  model: string(),
  ...
  get usage() {
    return optional(ChatGenerationTokenUsageSchema);
  },
}));
